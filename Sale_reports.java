...
import java.sql.*;
public class Sale_reports extends JFrame implements ActionListener,FocusListener
{
    JLabel l1,l2,l3;
    JTextField t1,t2;
    JTable table;
    ...
    public void actionPerformed(ActionEvent e) 
	{
        if(e.getSource()==b1) 
        {
            ...
            try 
			{
                ...
                model.setRowCount(0);
                String query="select bill_no,date,cust_name,model,sale_qty,amt from sale where str_to_date(date,'%d-%m-%Y') between str_to_date('"+start+"','%d-%m-%Y') and str_to_date('"+end+"','%d-%m-%Y')";
                ResultSet rs = stmt.executeQuery(query);
                while(rs.next()) 
				{
                    model.addRow(new Object[]{rs.getString("bill_no"),rs.getString("date"),rs.getString("cust_name"),rs.getString("model"),rs.getString("sale_qty"),rs.getString("amt")});
                }
            }
			catch(Exception ex) 
			{
