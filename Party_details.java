...
import java.sql.*;
public class Party_details extends JFrame implements ActionListener 
{
    JLabel l1,l2;
    JComboBox<String> c1;
    JTable table;
    ...
    public void loadData(String company) 
	{
		...
		{
            model.setRowCount(0);
            String query=company.equals("All")?"select d_id,d_name,company,mobile,balance from party":"select d_id,d_name,company,mobile,balance from party where company='"+company+"'";
            ResultSet rs=stmt.executeQuery(query);
            while(rs.next()) 
			{
                model.addRow(new Object[]{rs.getString("d_id"),rs.getString("d_name"),rs.getString("company"),rs.getString("mobile"),rs.getString("balance")});
            }
        } 
